...
- `refund_status` indicates whether the order is in an after-sale refund/return process
...
```json
{
  ...
  "paid_at": null,
  "completed_at": null,
  "cancelled_at": "2026-04-13T10:30:00+08:00",
  "received_at": null,
  "payment_notified_at": null,
  "created_at": "2026-04-13T10:00:00+08:00",
  "updated_at": "2026-04-13T10:30:00+08:00",
  "items": [
    {
      "product_id": 1,
      "product_name": "护眼贴",
...
- `services.order.rollback_order_shipment(order=..., operator=...)`
- `services.order.change_order_shipment(shipment=..., operator=..., courier_company=..., tracking_no=...)`
- `services.order.mark_order_received(order=...)`
...
- shipment status values are `unshipped`, `shipped`, and `received`
...
- order received ("签收") is triggered by KuaiDi100 push callback and transitions `shipment_status` from `shipped` to `received`
...
### Payment / Lifecycle Status (`status`)

| Value | Label | Description |
|-------|-------|-------------|
| `pending` | 待支付 | Order created, awaiting payment |
| `paid` | 已支付 | Payment confirmed |
| `completed` | 已完成 | Order fulfilled |
...
| Value | Label | Description |
|-------|-------|-------------|
| `unshipped` | 未发货 | Not yet shipped |
| `shipped` | 已发货 | Shipment confirmed |
| `received` | 已签收 | Customer signed for delivery |

### Refund / After-Sale Status (`refund_status`)

| Value | Label | Description |
|-------|-------|-------------|
| `none` | 无 | No after-sale request |
| `applied` | 申请中 | Refund/return requested |
| `approved` | 审核通过 | Refund/return approved |
| `rejected` | 已驳回 | Refund/return rejected |
| `cancelled` | 已取消 | After-sale request cancelled |
| `returned` | 已退货 | Goods returned |
| `refunded` | 已退款 | Refund completed |